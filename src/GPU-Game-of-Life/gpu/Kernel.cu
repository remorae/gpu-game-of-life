...
__global__ void updateGrid(unsigned char* grid, int intpitch, int width, int height)
{
	...
	const unsigned int iy = blockIdx.y * blockDim.y + threadIdx.y; //row index
	const unsigned int ix = blockIdx.x * blockDim.x + threadIdx.x; //column index
	...
	if (ty == 0)
	{
		...
		{
			// NE
			s_data[ty * sharedMemDim_x + tx + 2] = grid[((iy - 1 + height) % height) * intpitch + (ix + 1) % width];
		}
	}
	else if (ty == blockDim.y - 1)
	{
		...
		{
			// SE
			s_data[(ty + 2) * sharedMemDim_x + tx + 2] = grid[((iy + 1) % height) * intpitch + (ix + 1) % width];
		}
	}
	...
	{
		// W
		s_data[(ty + 1) * sharedMemDim_x + tx] = grid[iy * intpitch + (ix - 1 + width) % width];
	}
	...
	if ((tx == blockDim.x - 1) || (ix == width - 1))
	{
		// E
		if (ix == width - 1)
			s_data[(ty + 1) * sharedMemDim_x + tx + 2] = grid[iy * intpitch];
		else
			s_data[(ty + 1) * sharedMemDim_x + tx + 2] = grid[iy * intpitch + (ix + 1) % width];
	}
	
	// Itself
	s_data[(ty + 1) * sharedMemDim_x + tx + 1] = grid[iy * intpitch + ix];
